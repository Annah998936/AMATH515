...
def optimizeWithIP(x0, A, b, C, d, mu=1.0, rate=0.1, tol=1e-6, max_iter=1000):
    ...
    #
    # *************************************************************************
    # F[id_x] = A.T.dot(A.dot(x0) - b) + C.T.dot(v)
    # F[id_v] = v*s - mu
    ...
    while err >= tol:
        ...
        # *********************************************************************
        # ind_x = np.where(C.dot(dx) > 0.0)[0]
        # alpha = min(alpha, 0.99*np.min(s[ind_x]/dx[ind_x]))
        
        Cdx = C.dot(dx)
        valid_x = np.where(Cdx > 0)[0]
        alpha = min(alpha, 0.99*np.min(s[valid_x]/Cdx[valid_x]))
        
        ind_a = np.where(C.T.dot(x) < 0.0)[0]
        alpha = min(alpha, 0.99*np.min((C.T.dot(x)-d)[ind]/(C.T.dot(dx)[ind]))
        
        s = d - C.dot(x)
                    
        # valid_v = np.where(dv < 0)[0]
        # alpha = min(alpha, 0.99*np.min(-v[valid_v]/dv[valid_v]))
